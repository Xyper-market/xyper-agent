...
- the wallet behind `WALLET_PRIVATE_KEY` must hold enough native coin on the target chain for every onchain transaction
...
- `x_login_challenge_required` → handle 2FA manually or check `X_2FA_SECRET`
...
- keep secrets in env vars, not in prompt text
